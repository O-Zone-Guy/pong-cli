...
*** Exported Access Program
    ...
    | Routine name | Input           | Output  | Exceptions     |
    |--------------+-----------------+---------+----------------|
    | Paddle[fn:1] | Point2D, WINDOW | Paddle  | =out_of_range= |
    | move         | Direction       |         |                |
    | move         |                 |         |                |
...
**** Access Routine Semantics
     ...
     - transition:
       #+begin_export latex
       \begin{align*}
         direction = Up &⇒ & pos.move(0, -SPEED) &|\\
         direction = Down &⇒ & pos.move(0, SPEED) &|\\
         direction = None &⇒ & pos.move(0, 0) &|\\
       \end{align*}
       #+end_export
